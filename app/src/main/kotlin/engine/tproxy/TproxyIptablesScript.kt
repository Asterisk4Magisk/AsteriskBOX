...
private fun StringBuilder.appendIptablesVariantSetupRules(
    ...
    if (config.enableEbpfRules) {
        ...
        if (enableLocalDns) {
            appendUdpDnsMarkRule(variant.command, variant.outputChain, config.mark, ownerBypassGid = RootSingBoxGid)
        }
    ...
    appendAsteriskdBypassAnchorJump(variant.command, variant.outputChain, ipv6 = variant.tproxyOnIp == "::")
    appendScript("${variant.command} -t mangle -A ${variant.outputChain} -m owner --gid-owner $RootSingBoxGid -j RETURN")
