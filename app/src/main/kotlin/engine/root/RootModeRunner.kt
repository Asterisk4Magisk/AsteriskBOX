...
internal abstract class RootModeRunner<Config : RootModeStartConfig>(
    ...
    suspend fun prepareCoreLogFiles(coreLogPaths: SingBoxCoreLogPaths) = withContext(Dispatchers.IO) {
        val command = coreLogPaths.buildPrepareCoreLogFilesCommand()
        if (command.isBlank()) {
            return@withContext
        }
        runRootCommand(command, "Failed to prepare SingBox log files")
    }
    ...
    suspend fun ownsRuntime(runtimeLayout: RootRuntimeLayout): Boolean = withContext(Dispatchers.IO) {
        rootAccess.exec(
            buildRootProcessMatchCommand(
                ...
                executablePath = runtimeLayout.singBoxCorePath,
                uid = RootSingBoxUid,
                gid = RootSingBoxGid,
            ),
