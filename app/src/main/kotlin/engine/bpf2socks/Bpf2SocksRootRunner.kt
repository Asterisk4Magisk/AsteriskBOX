...
internal class Bpf2SocksRootRunner(
    ...
    override fun buildPostCoreStartCommand(config: Bpf2SocksStartConfig): String {
        ...
        return buildString {
            appendScript(
                ...
                $${setuidgid.shellQuote()} $${RootSingBoxUid.toString().shellQuote()} $${RootSingBoxGid.toString().shellQuote()} $${helper.executablePath.shellQuote()} --start --config $${helper.configPath.shellQuote()} --pid $${helper.pidPath.shellQuote()} >> $${config.root.coreLogPaths.errorLogPath.shellQuote()} 2>&1 < /dev/null &
    ...
    override fun buildReadinessCheck(config: Bpf2SocksStartConfig): RootReadinessCheck {
        ...
        return RootReadinessCheck(
            ...
            failureMessage = "singBox started but bpf2socks bridge port $bridgePort is not ready",
        )
    }
    ...
    override fun StringBuilder.appendStartupFailureDiagnostics(config: Bpf2SocksStartConfig) {
        ...
        appendScript(
            $$"""
                ...
                $${config.root.setuidgidPath.shellQuote()} $${RootSingBoxUid.toString().shellQuote()} $${RootSingBoxGid.toString().shellQuote()} $${config.controlPaths.executablePath.shellQuote()} --probe --config $${config.controlPaths.configPath.shellQuote()} 2>&1 || true
                echo
    ...
    return buildString {
        appendScript(
            ...
            for bpf2socks_pid in $bpf2socks_pid_candidates; do
                ...
                set -- $uid_line
                [ "$3" = "$${RootSingBoxUid}" ] || [ "$5" = "$${RootSingBoxUid}" ] || continue
                gid_line="$(grep '^Gid:' "$bpf2socks_pid_dir/status" 2>/dev/null || true)"
                set -- $gid_line
                [ "$3" = "$${RootSingBoxGid}" ] || [ "$5" = "$${RootSingBoxGid}" ] || continue
    ...
    return buildString {
        appendScript(
            ...
            (
              ...
              set -- $uid_line
              [ "$3" = "$${RootSingBoxUid}" ] || [ "$5" = "$${RootSingBoxUid}" ] || exit 1
              gid_line="$(grep '^Gid:' /proc/"$pid"/status 2>/dev/null || true)"
              set -- $gid_line
              [ "$3" = "$${RootSingBoxGid}" ] || [ "$5" = "$${RootSingBoxGid}" ] || exit 1
            )
            """,
        )
    }
