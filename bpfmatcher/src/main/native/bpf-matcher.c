...
#define PROBE_XT_OUTPUT_V4_PROGRAM_NAME "probe_xt_output_v4"
#define PROBE_XT_OUTPUT_V6_PROGRAM_NAME "probe_xt_output_v6"
#define PROBE_XT_PREROUTING_V4_PROGRAM_NAME "probe_xt_prerouting_v4"
#define PROBE_XT_PREROUTING_V6_PROGRAM_NAME "probe_xt_prerouting_v6"
...
static char active_xt_output_v4_program_path[MAX_PATH_LEN];
static char active_xt_output_v6_program_path[MAX_PATH_LEN];
static char active_xt_prerouting_v4_program_path[MAX_PATH_LEN];
static char active_xt_prerouting_v6_program_path[MAX_PATH_LEN];

static bool is_absolute_bounded_path(const char *path) {
    return path != NULL && path[0] == '/' && strnlen(path, MAX_PATH_LEN) < MAX_PATH_LEN;
}

static bool build_child_path(char *out, size_t out_size, const char *directory, const char *name) {
    if (out == NULL || out_size == 0U || !is_absolute_bounded_path(directory) || name == NULL || name[0] == '\0') {
        return false;
    }
...
static bool load_policy(const char *path, struct policy *policy) {
    ...
    if (!is_absolute_bounded_path(path)) {
        fprintf(stderr, "policy path must be absolute and shorter than %d bytes\n", MAX_PATH_LEN);
        return false;
    }
    ...
    bool program_paths_ok = json_string(
        json,
    ...
    free(json);
    if (!program_paths_ok ||
        !is_absolute_bounded_path(policy->xt_output_v4_program_path) ||
        !is_absolute_bounded_path(policy->xt_output_v6_program_path) ||
        !is_absolute_bounded_path(policy->xt_prerouting_v4_program_path) ||
        !is_absolute_bounded_path(policy->xt_prerouting_v6_program_path)) {
        fprintf(stderr, "policy requires four absolute pinned program paths\n");
        return false;
    }
...
static void cleanup_attaches(void) {
    if (active_xt_output_v4_program_path[0] != '\0') unlink(active_xt_output_v4_program_path);
    if (active_xt_output_v6_program_path[0] != '\0') unlink(active_xt_output_v6_program_path);
    if (active_xt_prerouting_v4_program_path[0] != '\0') unlink(active_xt_prerouting_v4_program_path);
    if (active_xt_prerouting_v6_program_path[0] != '\0') unlink(active_xt_prerouting_v6_program_path);
...
static int probe_json(const char *bpf_directory, bool need_ipv6) {
    char probe_xt_output_v4_program_path[MAX_PATH_LEN];
    char probe_xt_output_v6_program_path[MAX_PATH_LEN];
    char probe_xt_prerouting_v4_program_path[MAX_PATH_LEN];
    char probe_xt_prerouting_v6_program_path[MAX_PATH_LEN];
    if (!build_child_path(
            probe_xt_output_v4_program_path,
            sizeof(probe_xt_output_v4_program_path),
            bpf_directory,
            PROBE_XT_OUTPUT_V4_PROGRAM_NAME) ||
        !build_child_path(
            probe_xt_output_v6_program_path,
            sizeof(probe_xt_output_v6_program_path),
            bpf_directory,
            PROBE_XT_OUTPUT_V6_PROGRAM_NAME) ||
        !build_child_path(
            probe_xt_prerouting_v4_program_path,
            sizeof(probe_xt_prerouting_v4_program_path),
            bpf_directory,
            PROBE_XT_PREROUTING_V4_PROGRAM_NAME) ||
        !build_child_path(
            probe_xt_prerouting_v6_program_path,
            sizeof(probe_xt_prerouting_v6_program_path),
            bpf_directory,
            PROBE_XT_PREROUTING_V6_PROGRAM_NAME)) {
        fprintf(stderr, "--bpf-dir is invalid or too long\n");
        return 2;
    }
    if (mkdir(bpf_directory, 0700) != 0 && errno != EEXIST) {
        fprintf(stderr, "cannot create bpf directory: %s\n", strerror(errno));
        return 1;
    }
...
static int stop_matcher(const char *policy_path) {
    struct policy policy;
    if (!load_policy(policy_path, &policy)) return 2;
    snprintf(
...
int main(int argc, char **argv) {
    if (has_arg(argc, argv, "--probe")) {
        const char *bpf_directory = arg_value(argc, argv, "--bpf-dir");
        if (!is_absolute_bounded_path(bpf_directory)) {
            fprintf(stderr, "--bpf-dir is required and must be absolute\n");
            return 2;
        }
        const char *ipv6 = arg_value(argc, argv, "--ipv6");
        return probe_json(bpf_directory, ipv6 && strcmp(ipv6, "1") == 0);
    }
    if (has_arg(argc, argv, "--stop")) {
        const char *policy = arg_value(argc, argv, "--policy");
        if (!is_absolute_bounded_path(policy)) {
            fprintf(stderr, "--policy is required and must be absolute\n");
            return 2;
        }
        return stop_matcher(policy);
    }
    ...
    fprintf(stderr, "Usage: %s --probe [--json] --bpf-dir DIR [--ipv6 0|1] | --start --policy FILE | --stop --policy FILE\n", argv[0]);
    return 2;
}
